...
<cfset idlist="2059896800093889,5168306077281773,7570038743238473,4957686695268887,4957686695268887,5656845852938153,4939942668444045,4011229145604540,
6717239641963456,2113264712719746,513220477112201,6124646274859299,4718663163266575,4751269202013823,9196068365305411,287632479529517,3915226238946704,2079644992618586,1643549737839785,5447900290690089,9457234687056194,6013464740111584,8969708858431755,8940071724993764,8397709210207872,
9585983862567406,5077292562212580,1652353865637104,1825263940814973,9616825572719411,3095449269753141,6962738377866021,145021333590586,
2838105322279545,8609926619731182,3510772137080679">
...
<cfset link="http://api.indeed.com/ads/apisearch?publisher=#trim(variables.publisher)#&v=2&q=#variables.q#&l=#variables.l#&sr=directhire&st=employer&limit=#variables.qt#&start=#variables.start#&fromage=#variables.fromage#&co=#variables.co#&salary=#variables.salary#&sort=#variables.sb#&filter=1&userip=#cgi.REMOTE_ADDR#&useragent=#cgi.HTTP_USER_AGENT#">
<cfelse>

<cfset link="http://api.indeed.com/ads/apisearch?publisher=#trim(variables.publisher)#&v=2&q=#variables.q#&l=#variables.l#&st=#variables.st#&start=#variables.start#&limit=#variables.qt#&fromage=#variables.fromage#&salary=#variables.salary#&sort=#variables.sb#&filter=1&latlong=1&co=#variables.co#&chnl=&radius=#variables.radius#&filter=1&userip=#cgi.REMOTE_ADDR#&useragent=#cgi.HTTP_USER_AGENT#">
</cfif>
...
<cfif xml.response.totalresults.xmltext is "0">
	...
	hint="I create/initilize a 1 row query to to force datatyping ">	
		...
		<cfset columnlist = "title,source,city,state,country,location,employer,publish_date,description,link,jobid,publisheddisplay,latitude,longitude,sponsored">
		<!--- seupt datatypes for our columns --->
		<cfset typelist = "str,str,str,str,str,str,str,str,str,str,str,str,str,str,str">
		<cfset str = "">
		<cfset date = Now()>
